...
print(df.loc[df.item_name == 'Chicken Salad Bowl'].reset_index(drop = True).head())

#08 new_price값이 9 이하이고 item_name 값이 Chicken Salad Bowl 인 데이터 프레임을 추출하라
print(df.loc[(df.item_name == 'Chicken Salad Bowl') & (df.new_price<=9)].head())
...
print(df.sort_values('new_price', ascending = False).reset_index(drop = True).head())
...
NewD = df.loc[(df.item_name =='Steak Salad')|(df.item_name == 'Bowl')]
print(NewD.drop_duplicates('item_name'))

#15 df의 item_name 컬럼 값이 Steak Salad 또는 Bowl 인 데이터를 데이터 프레임화 한 후, item_name를 기준으로 중복행이 있으면 제거하되 마지막 케이스만 남겨라
NewD = df.loc[(df.item_name == 'Steak Salad')|(df.item_name == 'Bowl')]
print(NewD.drop_duplicates('item_name', keep = 'last'))
...
NewD=df[df.choice_description.str.contains('Black')]
print(NewD['choice_description'].head())
