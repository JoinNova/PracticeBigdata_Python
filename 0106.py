...
lst = df[df['top year'] == 2015].artist.value_counts().reset_index()
#print(lst)
chk, ans = 0, []
for _ in lst.values:
    if _[1] >= chk:
...
lst = df[df['rank'] <= 10].value_counts('top genre').index[1]
print(lst)

#04 피처링의 경우 title에 표시된다. 피처링을 가장 많이 해준 가수는 누구인가?
lst = df[df.title.str.contains('\(feat')].title.str.split('\(feat.').str[1].str[1:-1].value_counts().reset_index()
#print(lst)
chk, ans = 0, []
#print(type(chk))
for _ in lst.values:
...
ans = df[df['top year'] != df['year released']].value_counts('top year').index[0]
print(int(ans))

#06 artist 컬럼의 값에 대소문자 상관없이 q 단어가 들어가는 아티스트는 몇명인가?
chk =df[df.artist.str.lower().str.contains('q')].artist.nunique()
print(chk)
...
chk = df[['artist type', 'artist']].value_counts().reset_index().artist.value_counts().index[0]
print(chk)
