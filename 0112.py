...
chk = df[df['rank'] <= 10]
print(chk)
ans = chk.value_counts('top genre').index[1]
print(ans)

#04 피처링의 경우 title에 표시된다. 피처링을 가장 많이 해준 가수는 누구인가?
ans = df.title.str.split('\(feat. ').str[1].str[:-1].value_counts().index[0]
print(ans)

#05 top year 년도를 기준으로 발매일(year released)과 top100에 진입한 일자 (top year)가 다른 곡의 숫자를 count 했을때 가장 많은 년도는?
chk = df[df['top year'] != df['year released']].value_counts('top year').index[0]
print(int(chk))
...
chk =df.groupby('top year').nrgy.mean().sort_values()
print(chk)
ans = abs(chk.values[0] - chk.values[-1])
print(ans)
