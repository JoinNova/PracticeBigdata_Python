...
#print(chk['Attack'])
#print(chk.head(400))
#print(chk.iloc[400:800])
ans = chk.head(400).value_counts('Legendary')[1] - chk.iloc[400:800].value_counts('Legendary')[1]
print(ans)
...
chk = df2.head(int(len(df2)*0.6))
#print(int(len(df2)*0.6))
ans = chk['Defense'].quantile(.25)
print(ans)

#04 Type 1 컬럼의 속성이 Fire인 포켓몬들의 Attack의 평균이상인 Water속성의 포켓몬 수를 구하여라
chk = df[df['Type 1'] =='Fire'].Attack.mean()
#print(chk)
ans = df[(df['Type 1'] == 'Water') & (df.Attack >= chk)]
print(len(ans))
...
#print(df['gap'])
chk = df.groupby(['Generation'])
print(chk.gap.mean().sort_values(ascending = False).index[0])
