...
ans = df.loc[df['quantity'] == 3].reset_index(drop = True)
print(ans.head())
...
ans = ans.loc[df.new_price <= 5]
print(len(ans))
#07 item_name명이 Chicken Salad Bowl 인 데이터 프레임을 추출하라고 index 값을라
...
print(ans)
#16 df의 데이터 중 new_price값이 new_price값의 평균값 이상을 가지는 데이터들을 인덱싱하라6
