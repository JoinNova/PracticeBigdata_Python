...
print(df[df.item_name == 'Chicken Salad Bowl'].reset_index(drop = True).head())
#08 new_price값이 9 이하이고 item_name 값이 Chicken Salad Bowl 인 데이터 프레임을 추출하라
print(df[(df.new_price <= 9) & (df.item_name == 'Chicken Salad Bowl')].head())
...
print(df[df.item_name.str.contains('Chips')]['item_name'].head())
...
print(df.loc[df.choice_description.str.contains('Black')]['choice_description'].head())
#21 df의 데이터 중 choice_description 값에 Vegetables 들어가지 않는 경우의 갯수를 출력하라
print(len(df.loc[~df.choice_description.str.contains('Vegetables')]['choice_description']))
